    ...
    <section class="w-full h-full flex items-center justify-center flex-col">
        ...
        <div class="w-11/12 h-full flex items-center cel:justify-around">
            ...
            <form class="w-full h-full flex items-center justify-center flex-col">
                <div class="w-11/12 h-20 bg-teal-400 flex items-center justify-start">
                    <input type="text" name="buscarpor" id="buscarpor" class=" text-center w-1/4 h-10 ml-5 rounded-lg border-2 border-black" placeholder="Buscar un cliente..." value="{{ $buscarpor }}">
                    <button type="submit" class="w-40 h-10 bg-green-500 text-white ml-5 rounded-lg">Buscar</button>
                </div>
                <div class="w-11/12 h-20">
                    <table class="w-full flex items-center justify-center flex-col">
                        <thead class="w-full flex items-center justify-center bg-teal-400 shadow-neutral-300 shadow-md mt-3 mb-3 h-24">
                            <tr class="w-full flex items-center justify-center text-center">
                                <th class="w-1/12 text-2xl "></th>
                                <th class="w-3/12 text-2xl ">Nombre</th>
                                <th class="w-3/12 text-2xl ">Apellido</th>
                                <th class="w-2/12 text-2xl ">Dni</th>
                                <th class="w-1/12 text-2xl ">Edad</th>
                                <th class="w-2/12 text-2xl"></th>
                            </tr>
                        </thead>
                        <tbody class="w-full flex items-center justify-center flex-col mt-">
                            @foreach ($aClientes as $clientes)
                            <tr class="shadow-neutral-300 shadow-md w-full flex items-center justify-center text-center bg-teal-200 my-3 h-20">
                                <td class="w-1/12 text-lg font-normal bg-teal-400 h-full grid place-items-center"><img src="{{ asset('imagenes/cliente.png') }}" alt="Cliente" class="cel:w-10 w-10"></td>
                                <td class="w-3/12 text-lg font-normal ">{{$clientes->nombre}}</td>
                                <td class="w-3/12 text-lg font-normal ">{{$clientes->apellido}}</td>
                                <td class="w-2/12 text-lg font-normal ">{{$clientes->dni}}</td>
                                <td class="w-1/12 text-lg font-normal ">{{$clientes->edad}}</td>
                                <td class="w-2/12 text-lg font-normal grid place-items-center">
